...
export interface PositionStats { // TODO: Review data structure
    assetId: string;
    type: PositionType;
...
export const positionsTransformer = (memberPools: MemberPool[], pools: PoolDetails) => {
    ...
    memberPools.forEach((memberPool) => {
      ...
      if (memberPool.pool.includes("/")) {
        ...
        const depositSaverValueAsset = baseToAsset(
          baseAmount(memberPool.assetAdded).minus(
        ...
        const depositValueRune = baseToAsset(
          baseAmount(memberPool.runeAdded).minus(
  ...
  const determinePositionType = (memberPool: MemberPool): PositionType => {
    ...
    if (memberPool.runeAdded !== "0" && memberPool.assetDeposit !== "0" || memberPool.runePending !== "0" || memberPool.assetPending !== "0") return PositionType.DLP;
    return PositionType.SLP;
  };
  
  const determineStatus = (memberPool: MemberPool) => {
    if (Number(memberPool.assetPending) > 0 || Number(memberPool.runePending) > 0) return PositionStatus.LP_POSITION_INCOMPLETE;
