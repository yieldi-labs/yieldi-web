  ...
  useEffect(() => {
    const fetchRuneBalance = async () => {
      ...
      try {
        ...
        const amountStr: string = balance?.coins.find(coin => coin.asset === 'THOR.RUNE' )?.amount || "0"; // A RUNE address can hold several assets, look for the RUNE id to display the correct balance.
