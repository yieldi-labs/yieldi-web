...
const LiquidityPositionsContext = createContext<LiquidityPositionsContextType | undefined>(undefined);

export const LiquidityPositionsProvider = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <LiquidityPositionsContext.Provider value={{
        positions,
        pools,
        markPositionAsPending,
        isPending,
...
export const useLiquidityPositions = () => {
  ...
  if (!context) {
    throw new Error('useLiquidityPositions must be used within a LiquidityPositionsProvider');
  }
  return context;
};
